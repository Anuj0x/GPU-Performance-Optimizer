"""
Modernized GPU Performance Advisor (GPA) Benchmarking Script

A comprehensive benchmarking and profiling tool for GPU performance analysis
using modern Python features and best practices.
"""
...
@dataclass
class Config:
    """Global configuration for GPA benchmarking."""
    iterations: int = 10
    debug: bool = False
    verbose: bool = False
    fast: bool = False
...
class TestCase:
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.versions) != len(self.version_names):
            raise ValueError(f"Versions and version_names must have same length for {self.name}")
...
class GPAError(Exception):
    """Custom exception for GPA-related errors."""
    pass
...
def run_command(command: List[str], capture_output: bool = True, check: bool = True) -> subprocess.CompletedProcess:
    ...
    Raises:
        GPAError: If command fails and check=True
    """
    try:
        ...
        raise GPAError(error_msg) from e
...
def cleanup(target: Optional[str] = None) -> None:
    """
    Clean and rebuild the project.

    Args:
        target: Optional make target to build
    """
    logger = logging.getLogger(__name__)
    ...
    try:
        ...
        make_cmd = ['make', '-j8']
        if target:
            make_cmd.extend([target, 'clean'])
            run_command(make_cmd[:-2], check=False)  # make target clean
            run_command(make_cmd[:-1] + [target], check=False)  # make target
        else:
            run_command(make_cmd)

        logger.debug("Rebuilt project")

    except GPAError as e:
        logger.warning(f"Build failed, continuing anyway: {e}")
